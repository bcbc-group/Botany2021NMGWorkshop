...
#LD prune
set.seed(1000)
snpset <- snpgdsLDpruning(genofile, ld.threshold=1, slide.max.n=5000)
...
pop_code <- add.gdsn(genofile, "/scratch/Botany2020NMGWorkshop/Genome_assembly/SNP_calling/popmap_PCA.txt")

#plot(tab$EV2, tab$EV1, xlab="eigenvector 2", ylab="eigenvector 1")

# Get sample id
sample.id <- read.gdsn(index.gdsn(genofile, "sample.id"))
pop_code <- scan("/scratch/Botany2020NMGWorkshop/Genome_assembly/SNP_calling/pop_PCA.txt", what=character())
table(pop_code)
